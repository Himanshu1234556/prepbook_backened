...
const notesFolder = 'files/notes'; // Folder for handwritten notes
    ...
    try {
        ...
        const filePromises = files.map(async (file) => {
            try {
                ...
                const match = trimmedFile.match(/^(\d+)\s*&\s*(\d+)/);
                ...
                if (isNaN(subjectId) || isNaN(unitNumber)) {
                ...
                const title = unitNumber === 0 ? 'Complete Notes' : `Unit ${unitNumber} HandWritten Notes`;
